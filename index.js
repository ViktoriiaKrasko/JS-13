...
function complexConvert(data) {
  ...
  for (const value in newObj) {
    if (typeof newObj[value] === "number") {
      ...
      result[value] = newObj[value] + 1;
    } else {
      result[value] = newObj[value]; // Якщо значення не є числом, просто копіюємо його у новий об'єкт без змін.
    }
  }
...
function searchParamsURL(url) {
  ...
  const newObj = urlObj.searchParams; // Отримання об'єкта URLSearchParams з властивості 'searchParams' об'єкта URL
  const dictionary = new Map(); // Створення порожнього словника для збереження параметрів пошуку
  for (const param of newObj) {
    dictionary.set(param[0], param[1]);
  } // Перебір кожного параметра пошуку з 'searchParams' та додавання їх до словника 'params'
  return dictionary; // Кожен 'param' - це масив, де [0] - ім'я параметра, а [1] - значення параметра
  // Повертаємо словник
}
...
function appendSearchParam(url, key, value) {
  ...
  urlObj.searchParams.set(key, value); // Додаємо новий параметр пошуку до URL-адреси.
  return urlObj.toString(); // Повертаємо нову URL-адресу в рядковому форматі.
}
...
function processURL(url, options) {
  ...
  if (options.searchParams) {
    for (const key in options.searchParams) {
      // Якщо в опціях вказано параметри пошуку, додаємо їх до URL.// За допомогою for in перебираємо ключи параметрів пошуку
  ...
  return urlObj.toString(); // Повертаємо об'єкт URL у вигляді рядка.
}
...
function manipulateQuery(url, options) {
  ...
  if (options.get("append")) {
    ...
    for (const [key, value] of options.get("append")) {
      // ...перебираємо його ключі та значення за допомогою циклу for...of.// Додаємо кожний ключ і значення до об'єкта `searchParams` в URL.
...
function sortUrlParams(url) {
  const urlObj = new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
  const entries = [...urlObj.searchParams.entries()]; // Отримуємо масив з ключами і значеннями параметрів за допомогою методу 'entries'.
  entries.sort((a, b) => b[1] - a[0]); // Сортуємо масив за ключами у порядку зростання.
  urlObj.search = ""; // Очищуємо пошукові параметри URL.
  urlObj.searchParams.append(entries[1], entries[0]); // Додаємо відсортовані параметри до URL.
  return urlObj.toString(); // Повертаємо новий URL як рядок.
}
...
function getURLValues(url) {
  ...
  const array = []; // Масив для збереження значень пошукових параметрів.
  for (const key of keys) {
    array.push(...params.getAll(key));
  ...
  return array; // Повертаємо масив значень пошукових параметрів.
}
...
function getUrlKeys(url) {
  const urlObj = new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
  const array = [...urlObj.searchParams.keys()]; // Отримуємо масив зі всіма ключами пошукових параметрів за допомогою методу 'keys'.
  return array; // Повертаємо масив з ключами.
}
